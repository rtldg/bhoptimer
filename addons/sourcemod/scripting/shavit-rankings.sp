...
public Action Command_RecalcAll(int client, int args)
{
	...
	for(int i = 0; i < gI_Styles; i++)
	{
		...
		{
			FormatEx(sQuery, sizeof(sQuery), "UPDATE %splayertimes SET points = 0 WHERE style = %d;", gS_MySQLPrefix, i);
		}
		else
		{
			FormatEx(sQuery, sizeof(sQuery),
				...
				gS_MySQLPrefix, gS_MySQLPrefix, i, gCV_PointsPerTier.FloatValue, fMultiplier
			);
		}
...
void RecalculateMap(const char[] map, const int track, const int style, bool restOfTheBonuses=false)
{
	...
	char trackStr[20];
	FormatEx(trackStr, sizeof(trackStr), (restOfTheBonuses ? "> 1" : "= %d"), track);
	FormatEx(sQuery, sizeof(sQuery),
		"UPDATE %splayertimes AS PT, "...
		"( SELECT MIN(time) as time, map, track, style "...
		"	FROM %splayertimes "...
		"	WHERE style = %d and track %s and map = '%s'"...
		"	GROUP BY map, track, style "...
		") as WR "...
		"SET PT.points = GetRecordPoints(PT.style, PT.track, PT.time, PT.map, %.1f, %.3f, WR.time) "...
		"WHERE PT.style = WR.style and PT.track = WR.track and PT.map = WR.map;;",
		gS_MySQLPrefix, gS_MySQLPrefix, style, trackStr, map, gCV_PointsPerTier.FloatValue, StringToFloat(multiplier)
	);
